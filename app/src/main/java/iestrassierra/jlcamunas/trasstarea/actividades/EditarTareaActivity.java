...
public class EditarTareaActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.tareaEditable = getIntent().getParcelableExtra("TareaEditable");
        ...
        tareaViewModel.setFechaCreacion(tareaEditable.getStringFechaCreacion());
        tareaViewModel.setFechaObjetivo(tareaEditable.getStringFechaObjetivo());
        ...
        tareaViewModel.setRutaDocumento(tareaEditable.getRutaDocumento());
        tareaViewModel.setRutaImagen(tareaEditable.getRutaImagen());
        tareaViewModel.setRutaAudio(tareaEditable.getRutaAudio());
        tareaViewModel.setRutaVideo(tareaEditable.getRutaVideo());
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        int fragmentID = Objects.requireNonNull(getSupportFragmentManager().
                findFragmentById(R.id.contenedor_frag_editar)).getId();
    ...
    public void onBotonGuardarClicked() {
        ...
        //Editamos los valores de la tarea Editable
        if(titulo.compareTo(tareaEditable.getTitulo()) != 0)
            tareaEditable.setTitulo(titulo);
        if(fechaCreacion.compareTo(tareaEditable.getStringFechaCreacion()) != 0)
            tareaEditable.setFechaCreacion(fechaCreacion);
        if(fechaObjetivo.compareTo(tareaEditable.getStringFechaObjetivo()) != 0)
            tareaEditable.setFechaObjetivo(fechaObjetivo);
        if(!Objects.equals(progreso, tareaEditable.getProgreso()))
            tareaEditable.setProgreso(progreso);
        if(prioritaria != tareaEditable.isPrioritaria())
            tareaEditable.setPrioritaria(prioritaria);

        //Hacemos las comparaciones teniendo cuidado con los posibles null
        if (descripcion != null && (tareaEditable.getDescripcion() == null || !descripcion.equals(tareaEditable.getDescripcion()))) {
            tareaEditable.setDescripcion(descripcion);
        }

        if (rutaDocumento != null && (tareaEditable.getRutaDocumento() == null || !rutaDocumento.equals(tareaEditable.getRutaDocumento()))) {
            tareaEditable.setRutaDocumento(rutaDocumento);
        }

        if (rutaImagen != null && (tareaEditable.getRutaImagen() == null || !rutaImagen.equals(tareaEditable.getRutaImagen()))) {
            tareaEditable.setRutaImagen(rutaImagen);
        }

        if (rutaAudio != null && (tareaEditable.getRutaAudio() == null || !rutaAudio.equals(tareaEditable.getRutaAudio()))) {
            tareaEditable.setRutaAudio(rutaAudio);
        }

        if (rutaVideo != null && (tareaEditable.getRutaVideo() == null || !rutaVideo.equals(tareaEditable.getRutaVideo()))) {
            tareaEditable.setRutaVideo(rutaVideo);
        }


        //Creamos un intent de vuelta para la actividad Listado
        Intent aListado = new Intent();
        //Adjuntamos la tarea editada
        aListado.putExtra("TareaEditada", tareaEditable);
    ...
    public void cambiarFragmento(Fragment fragment){
        if (!fragment.isAdded()) {
            fragmentManager.beginTransaction()
                    .replace(R.id.contenedor_frag_editar, fragment)
                    .commit();
        }
    }
}